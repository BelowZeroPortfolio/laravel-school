...
@php
    $activeClasses = match($activeColor) {
        'green' => 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300',
        'red' => 'bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300',
        default => 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300',
    };
    
    $activeIconClasses = match($activeColor) {
...
<div class="mb-1" x-data="{ tooltip: false }">
    <a href="{{ $href }}" 
       ...
       class="flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-xl transition-all relative {{ $active ? $activeClasses : 'text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700/50' }}"
       :class="sidebarCollapsed ? 'justify-center' : ''">
        <svg class="w-5 h-5 flex-shrink-0 {{ $active ? $activeIconClasses : 'text-gray-400 dark:text-gray-500' }}" 
        ...
        <div x-show="tooltip" 
             x-transition 
             class="fixed ml-2 px-2 py-1 bg-gray-900 text-white text-xs rounded whitespace-nowrap z-[9999] pointer-events-none shadow-lg"
             :style="'left: ' + ($el.closest('aside').offsetWidth + 8) + 'px'">
            {{ $label }}
        </div>
    </a>
</div>
