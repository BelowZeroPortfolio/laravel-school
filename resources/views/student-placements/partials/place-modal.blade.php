...
<div x-show="showPlaceModal"
    ...
    <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
        <div class="relative transform overflow-hidden rounded-lg bg-white dark:bg-gray-800 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg"
             @click.away="showPlaceModal = false">
            ...
            <div class="px-4 py-4 sm:px-6 border-b border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between">
                    ...
                    <button @click="showPlaceModal = false" 
                            type="button" 
            ...
            <!-- Body -->
            <form method="POST" action="{{ route('student-placements.place') }}">
                @csrf
                <div class="px-4 py-4 sm:px-6 space-y-4">
                    <!-- Student Info -->
                    <div class="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4">
                        <p class="text-sm text-gray-500 dark:text-gray-400">Student</p>
                        <p class="text-lg font-medium text-gray-900 dark:text-white" x-text="placeStudent?.name"></p>
                    </div>

                    <input type="hidden" name="student_id" :value="placeStudent?.id">
                    ...
                    <!-- Reason -->
                    <div>
                        <label for="place_reason" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            Reason (Optional)
                        </label>
                        <textarea name="reason" 
                                  ...
                                  placeholder="Enter reason for placement..."></textarea>
                    </div>
                </div>
                ...
                <div class="px-4 py-3 sm:px-6 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700 flex justify-end space-x-2">
                    <x-button type="button" variant="outline" @click="showPlaceModal = false">
                        Cancel
                    </x-button>
